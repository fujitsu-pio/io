 ...
 * サービスコレクションの情報を管理する.
 */
public class DebugResourceSourceManager implements ISourceManager {
    /** ログオブジェクト. */
    private static Logger log = LoggerFactory.getLogger(DebugResourceSourceManager.class);

    /**
     * コンストラクタ.
     */
    public DebugResourceSourceManager() {
    }
    ...
    public String getSource(String sourceName) throws DcEngineException {
        try {
            URL path = getClass().getResource("/js-lib/debug/" + sourceName);

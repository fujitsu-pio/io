...
public class MoveServiceSourceAccessControlTest extends JerseyTest {
     ...
     * コンストラクタ.
     */
    public MoveServiceSourceAccessControlTest() {
    ...
    /**
     * 正しいトークンを指定した場合ServiceSourceのMOVEができること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 正しいトークンを指定した場合ServiceSourceのMOVEができること() throws JAXBException {
        String token;
    ...
    /**
     * 不正なトークンを指定した場合ServiceSourceのMOVEで401エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 不正なトークンを指定した場合ServiceSourceのMOVEで401エラーとなること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動元Collectionにwrite権限を持つアカウントでServiceSourceのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元Collectionにwrite権限を持つアカウントでServiceSourceのMOVEをした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動元Collectionにwrite権限を含むアカウントでServiceSourceのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元Collectionにwrite権限を含むアカウントでServiceSourceのMOVEをした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動元Collectionにall権限を持つアカウントでServiceSourceのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元Collectionにall権限を持つアカウントでServiceSourceのMOVEをした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動元Collectionに権限を持たないアカウントでServiceSourceのMOVEをした場合403エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元Collectionに権限を持たないアカウントでServiceSourceのMOVEをした場合403エラーとなること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動先Collectionにread権限を持つアカウントでServiceSourceのMOVEをした場合403エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動先Collectionにread権限を持つアカウントでServiceSourceのMOVEをした場合403エラーとなること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動先Collectionにwrite権限を持つアカウントでServiceSourceのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動先Collectionにwrite権限を持つアカウントでServiceSourceのMOVEをした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動先Collectionにwrite権限を含むアカウントでServiceSourceのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動先Collectionにwrite権限を含むアカウントでServiceSourceのMOVEをした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動先Collectionにall権限を持つアカウントでServiceSourceのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動先Collectionにall権限を持つアカウントでServiceSourceのMOVEをした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動先Collectionに権限を持たないアカウントでServiceSourceのMOVEをした場合403エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動先Collectionに権限を持たないアカウントでServiceSourceのMOVEをした場合403エラーとなること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
    ...
    /**
     * 移動元の親Collectionにwrite権限が設定されている場合ServiceSourceのMOVEができること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親Collectionにwrite権限が設定されている場合ServiceSourceのMOVEができること() throws JAXBException {
        String token;
        String parentCol = "collection";
        String path = String.format("%s/%s/%s/__src/%s", BOX_NAME, parentCol, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
        ...
        try {
            ...
            DavResourceUtils.createServiceCollection(AbstractCase.BEARER_MASTER_TOKEN, HttpStatus.SC_CREATED,
                    CELL_NAME, BOX_NAME, parentCol + "/" + SRC_COL_NAME);
            DavResourceUtils.createWebDavFile(MASTER_TOKEN, CELL_NAME, path, FILE_BODY, MediaType.TEXT_PLAIN,
                    HttpStatus.SC_CREATED);

            setDefaultAcl(parentCol);
            setPrincipalAllAcl(DST_COL_NAME);
            ...
            DavResourceUtils.deleteCollection(CELL_NAME, BOX_NAME, parentCol + "/" + SRC_COL_NAME, MASTER_TOKEN, -1);
            DavResourceUtils.deleteCollection(CELL_NAME, BOX_NAME, parentCol, MASTER_TOKEN, -1);
            DavResourceUtils.deleteWebDavFile(CELL_NAME, MASTER_TOKEN, BOX_NAME, DST_COL_NAME + "/__src/" + FILE_NAME);
        }
    }

    /**
     * 移動元の親Collectionに権限を持たないアカウントでServiceSourceのMOVEをした場合403エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親Collectionに権限を持たないアカウントでServiceSourceのMOVEをした場合403エラーとなること() throws JAXBException {
        String token;
        String parentCol = "collection";
        String path = String.format("%s/%s/%s/__src/%s", BOX_NAME, parentCol, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, "__src", FILE_NAME);
        ...
        try {
            ...
            DavResourceUtils.deleteWebDavFile(CELL_NAME, MASTER_TOKEN, BOX_NAME, parentCol + "/" + SRC_COL_NAME
                    + "/__src/" + FILE_NAME);
            DavResourceUtils.deleteCollection(CELL_NAME, BOX_NAME, parentCol + "/" + SRC_COL_NAME, MASTER_TOKEN, -1);
            DavResourceUtils.deleteCollection(CELL_NAME, BOX_NAME, parentCol, MASTER_TOKEN, -1);
        }
    ...
    /**
     * 移動先の親Collectionにwrite権限が設定されている場合ServiceSourceのMOVEができること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動先の親Collectionにwrite権限が設定されている場合ServiceSourceのMOVEができること() throws JAXBException {
        String token;
        String parentCol = "collection";
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, parentCol, DST_COL_NAME, "__src", FILE_NAME);
        ...
        try {
            ...
            DavResourceUtils.createWebDavCollection(MASTER_TOKEN, HttpStatus.SC_CREATED, CELL_NAME, BOX_NAME,
                    parentCol);
            DavResourceUtils.createServiceCollection(AbstractCase.BEARER_MASTER_TOKEN, HttpStatus.SC_CREATED,
                    CELL_NAME, BOX_NAME, parentCol + "/" + DST_COL_NAME);
            DavResourceUtils.createWebDavFile(MASTER_TOKEN, CELL_NAME, path, FILE_BODY, MediaType.TEXT_PLAIN,
                    HttpStatus.SC_CREATED);
            ...
            DavResourceUtils.deleteWebDavFile(CELL_NAME, MASTER_TOKEN, BOX_NAME, parentCol + "/" + DST_COL_NAME
                    + "/__src/" + FILE_NAME);
            DavResourceUtils.deleteCollection(CELL_NAME, BOX_NAME, parentCol + "/" + DST_COL_NAME, MASTER_TOKEN, -1);
            DavResourceUtils.deleteCollection(CELL_NAME, BOX_NAME, parentCol, MASTER_TOKEN, -1);
        }
    }

    /**
     * 移動先の親Collectionに権限を持たないアカウントでServiceSourceのMOVEをした場合403エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動先の親Collectionに権限を持たないアカウントでServiceSourceのMOVEをした場合403エラーとなること() throws JAXBException {
        String token;
        String parentCol = "collection";
        String path = String.format("%s/%s/__src/%s", BOX_NAME, SRC_COL_NAME, FILE_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, parentCol, DST_COL_NAME, "__src", FILE_NAME);

...
public class MoveServiceCollectionAclTest extends JerseyTest {
     ...
     * コンストラクタ.
     */
    public MoveServiceCollectionAclTest() {
    ...
    /**
     * 正しいトークンを指定した場合ServiceコレクションのMOVEができること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 正しいトークンを指定した場合ServiceコレクションのMOVEができること() throws JAXBException {
        String token;
    ...
    /**
     * 不正なトークンを指定した場合ServiceコレクションのMOVEで401エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 不正なトークンを指定した場合ServiceコレクションのMOVEで401エラーとなること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
        String srcColPath = SRC_COL_NAME + "/" + COL_NAME;
    ...
    /**
     * 移動元の親がread権限を持つアカウントでServiceコレクションをMOVEしても403エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親がread権限を持つアカウントでServiceコレクションをMOVEしても403エラーとなること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
        String srcColPath = SRC_COL_NAME + "/" + COL_NAME;
    ...
    /**
     * 移動元の親がwrite権限を持つアカウントでServiceコレクションをMOVEした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親がwrite権限を持つアカウントでServiceコレクションをMOVEした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
        String srcColPath = SRC_COL_NAME + "/" + COL_NAME;
    ...
    /**
     * 移動元の親がwrite権限を含むアカウントでServiceコレクションのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親がwrite権限を含むアカウントでServiceコレクションのMOVEをした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
        String srcColPath = SRC_COL_NAME + "/" + COL_NAME;
    ...
    /**
     * 移動元の親にall権限を持つアカウントでServiceコレクションのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親にall権限を持つアカウントでServiceコレクションのMOVEをした場合201となること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
        String srcColPath = SRC_COL_NAME + "/" + COL_NAME;
    ...
    /**
     * 移動元の親に権限を持たないアカウントでServiceコレクションのMOVEをした場合403エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親に権限を持たないアカウントでServiceコレクションのMOVEをした場合403エラーとなること() throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
        String srcColPath = SRC_COL_NAME + "/" + COL_NAME;
    ...
    /**
     * 移動元の親の親がwrite権限を持つアカウントでServiceコレクションのMOVEをした場合に201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親の親がwrite権限を持つアカウントでServiceコレクションのMOVEをした場合に201となること() throws JAXBException {
        String token;
    ...
    /**
     * 移動元の親の親が権限を持たないアカウントでServiceコレクションのMOVEをした場合に403エラーとなること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親の親が権限を持たないアカウントでServiceコレクションのMOVEをした場合に403エラーとなること() throws JAXBException {
        String token;
        String collection = "collection";
        String path = String.format("%s/%s/%s/%s", BOX_NAME, SRC_COL_NAME, collection, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
    ...
    /**
     * 移動元の親にwrite権限を持つ_かつ_移動対象にread権限を持つアカウントでServiceコレクションのMOVEをした場合201となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親にwrite権限を持つ_かつ_移動対象にread権限を持つアカウントでServiceコレクションのMOVEをした場合201となること()
    ...
    /**
     * 移動元の親にread権限を持つ_かつ_移動対象にwrite権限を持つアカウントでServiceコレクションのMOVEをした場合403となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親にread権限を持つ_かつ_移動対象にwrite権限を持つアカウントでServiceコレクションのMOVEをした場合403となること()
            throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
     ...
     */
    @Test
    public void 移動元の親にread権限を持つ_かつ_移動対象にread権限を持つアカウントでServiceコレクションのMOVEをした場合403となること()
            throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
     ...
     */
    @Test
    public void 移動元の親にwrite権限を持つ_かつ_移動先の親にwrite権限を持つアカウントでServiceコレクションのMOVEをした場合201となること()
            throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
    ...
    /**
     * 移動元の親にread権限を持つ_かつ_移動先の親にread権限を持つアカウントでServiceコレクションのMOVEをした場合403となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親にread権限を持つ_かつ_移動先の親にread権限を持つアカウントでServiceコレクションのMOVEをした場合403となること()
            throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
    ...
    /**
     * 移動元の親にread権限を持つ_かつ_移動先の親にwrite権限を持つアカウントでServiceコレクションのMOVEをした場合403となること.
     * @throws JAXBException ACLのパース失敗
     */
    @Test
    public void 移動元の親にread権限を持つ_かつ_移動先の親にwrite権限を持つアカウントでServiceコレクションのMOVEをした場合403となること()
            throws JAXBException {
        String token;
        String path = String.format("%s/%s/%s", BOX_NAME, SRC_COL_NAME, COL_NAME);
        String destination = UrlUtils.box(CELL_NAME, BOX_NAME, DST_COL_NAME, COL_NAME);
